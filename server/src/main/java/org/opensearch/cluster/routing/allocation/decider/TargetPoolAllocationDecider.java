...
public class TargetPoolAllocationDecider extends AllocationDecider {
    ...
    public Decision canAllocate(ShardRouting shardRouting, RoutingNode node, RoutingAllocation allocation) {
        ...
        if (shardPool != targetNodePool) {
            ...
            return allocation.decision(Decision.NO, NAME,
                "Routing pools are incompatible. Shard pool: [%s], Node Pool: [%s]", shardPool, targetNodePool);
        }
        return allocation.decision(Decision.YES, NAME,
            "Routing pools are compatible. Shard pool: [%s], Node Pool: [%s]", shardPool, targetNodePool);
    }
    ...
    private Decision canAllocateInTargetPool(IndexMetadata indexMetadata, DiscoveryNode node, RoutingAllocation allocation) {
        ...
        if (indexPool != targetNodePool) {
            ...
            return allocation.decision(Decision.NO, NAME,
                "Routing pools are incompatible. Index pool: [%s], Node Pool: [%s]", indexPool, targetNodePool);
        }
        return allocation.decision(Decision.YES, NAME,
            "Routing pools are compatible. Index pool: [%s], Node Pool: [%s]", indexPool, targetNodePool);
    }
