...
public class StoreTests extends OpenSearchTestCase {
    ...
    public void testRefCount() {
        ...
        Store store = new LocalOnlyStore(shardId, indexSettings, StoreTests.newDirectory(random()), new DummyShardLock(shardId));
    ...
    public void testNewChecksums() throws IOException {
        final ShardId shardId = new ShardId("index", "_na_", 1);
        Store store = new LocalOnlyStore(shardId, INDEX_SETTINGS, StoreTests.newDirectory(random()), new DummyShardLock(shardId));
    ...
    public void testRecoveryDiff() throws IOException, InterruptedException {
        ...
        Store store;
        {
            ...
            iwc.setUseCompoundFile(random.nextBoolean());
            final ShardId shardId = new ShardId("index", "_na_", 1);
            store = new LocalOnlyStore(shardId, INDEX_SETTINGS, StoreTests.newDirectory(random()), new DummyShardLock(shardId));
            IndexWriter writer = new IndexWriter(store.directory(), iwc);
            final boolean lotsOfSegments = rarely(random);
            for (Document d : docs) {
                writer.addDocument(d);
    ...
    public void testOnCloseCallback() throws IOException {
        ...
        Store store = new LocalOnlyStore(shardId, INDEX_SETTINGS, StoreTests.newDirectory(random()), lock, theLock -> {
    ...
    public void testStoreStats() throws IOException {
        ...
        Settings settings = Settings.builder()
            ...
            .build();
        Store store = new LocalOnlyStore(
            shardId,
    ...
    public void testCanOpenIndex() throws IOException {
        ...
        Store store = new LocalOnlyStore(shardId, INDEX_SETTINGS, dir, new DummyShardLock(shardId));
    ...
    public void testCorruptionMarkerVersionCheck() throws IOException {
        ...
        try (Store store = new LocalOnlyStore(shardId, INDEX_SETTINGS, dir, new DummyShardLock(shardId))) {
    ...
    public void testEnsureIndexHasHistoryUUID() throws IOException {
        final ShardId shardId = new ShardId("index", "_na_", 1);
        try (Store store = new LocalOnlyStore(shardId, INDEX_SETTINGS, StoreTests.newDirectory(random()), new DummyShardLock(shardId))) {
    ...
    public void testGetPendingFiles() throws IOException {
        ...
        try (Store store = new LocalOnlyStore(shardId, INDEX_SETTINGS, new NIOFSDirectory(createTempDir()), new DummyShardLock(shardId))) {
    ...
    public void testGetMetadataWithSegmentInfos() throws IOException {
        final ShardId shardId = new ShardId("index", "_na_", 1);
        Store store = new LocalOnlyStore(shardId, INDEX_SETTINGS, new NIOFSDirectory(createTempDir()), new DummyShardLock(shardId));
        store.createEmpty(Version.LATEST);
        SegmentInfos segmentInfos = Lucene.readSegmentInfos(store.directory());
    ...
    public void testReadSegmentsFromOldIndices() throws Exception {
        ...
        try {
            ...
            store = new LocalOnlyStore(shardId, indexSettings, StoreTests.newMockFSDirectory(tmp), new DummyShardLock(shardId));
    ...
    public void testReadSegmentsFromOldIndicesFailure() throws IOException {
        ...
        Store store = new LocalOnlyStore(shardId, indexSettings, StoreTests.newMockFSDirectory(tmp), new DummyShardLock(shardId));
